...
namespace Dotmim.Sync.Tests.Misc
{
    public static class HelperDatabase
    {
        ...
        /// <summary>
        /// Oracle identity sequences do not advance when EF Core seeds rows with explicit
        /// literal ids, so the first generated value would collide with a seeded id
        /// (ORA-00001). Restart every identity column past the current data
        /// (START WITH LIMIT VALUE). SQL Server and MySQL advance their counters on
        /// explicit inserts, so only Oracle needs this.
        /// </summary>
        internal static async Task ResetOracleIdentitySequencesAsync(string dbName)
        {
            using var connection = new OracleConnection(GetOracleDatabaseConnectionString(dbName));
            await connection.OpenAsync().ConfigureAwait(false);

            var identityColumns = new List<(string TableName, string ColumnName)>();

            using (var listCommand = connection.CreateCommand())
            {
                listCommand.CommandText = "SELECT TABLE_NAME, COLUMN_NAME FROM USER_TAB_IDENTITY_COLS";
                using var reader = await listCommand.ExecuteReaderAsync().ConfigureAwait(false);
            ...
            foreach (var (tableName, columnName) in identityColumns)
            {
                ...
                alterCommand.CommandText = $"ALTER TABLE \"{tableName}\" MODIFY (\"{columnName}\" GENERATED BY DEFAULT AS IDENTITY (START WITH LIMIT VALUE))";
