...
4. **ODP.NET dual-API output rule (found by Task 2 quality review):** output parameter values come back as PROVIDER types (`OracleDecimal`) when the parameter type was set via `OracleDbType`, and as .NET types (boxed `int`) when set via `DbType`. The orchestrator hard-casts `(int)Value` on `sync_row_count` — so that parameter MUST be typed via `parameter.DbType = DbType.Int32` (landed in `e949ef1b` as `AddSyncRowCountParameter`). This is a contract, not folklore: never type output parameters via `OracleDbType`.
   Additional Task 2 hardening in the same commit: filter parameters never map to CLOB (illegal in comparisons, ORA-00932), guid-strings on Raw parameters without `SourceColumn` parse as Guid before the base64 fallback, converter-produced booleans coerce to 0/1, and the strip regex uses `(?![\w$#])` instead of `\b` (`#`/`$` are legal Oracle identifier chars).
   Task 16 additions: confirm `sync_row_count` reads back as boxed `int` under load; exercise a `DbType.Time` column (TimeSpan→TIMESTAMP bind is a known pre-existing mapping tension) and a `DbType.Date` column on net6+ (`DateOnly` predates ODP.NET 3.21 support).
